...
function CommentItem({
  ...
  const canAct = comment.author.id === currentUserId || isAdmin;
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          <Avatar className="size-8">
            {comment.author.photo_url && <AvatarImage src={comment.author.photo_url} />}
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {comment.edited_at && <span className="ml-1">(Editado)</span>}
            </p>
          </div>
        </div>
        {canAct && !editing && (
...
export function CommentsList({ aulaId, className }: CommentsListProps) {
  ...
  const createMut = useMutation({
    mutationFn: (text: string) => createComentario(aulaId, text),
    onMutate: async (text) => {
  ...
  const editMut = useMutation({
    mutationFn: ({ id, text }: { id: string; text: string }) => updateComentario(id, text),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["comentarios", aulaId] }),
  ...
  const deleteMut = useMutation({
    ...
    onMutate: async (id) => {
      ...
      queryClient.setQueryData<Comentario[]>(["comentarios", aulaId], (old) =>
        (old ?? []).map((c) => c.id === id ? { ...c, deleted_at: new Date().toISOString() } : c),
      );
      return { prev };
    },
    onError: (_err, _vars, ctx) => {
