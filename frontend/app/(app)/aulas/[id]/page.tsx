...
import { CheckCircle2, Circle, Clock, Trophy, ArrowRight, ChevronLeft } from "lucide-react";
...
function nextAula(trilha: TrilhaDetalhe, currentAulaId: string) {
  const all = trilha.modulos.flatMap((m) => m.aulas);
  ...
  const toggleMut = useMutation({
    ...
    onMutate: async () => {
      ...
      qc.setQueryData(["aula", id], (old: typeof aula) =>
        old ? { ...old, concluida: !old.concluida } : old,
      );
      return { prev };
    },
    onError: (_e, _v, ctx) => {
        ...
        {next && (
          <GlassCard variant="soft" className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm font-medium line-clamp-1">{next.titulo}</p>
            </div>
            <Link
              href={`/aulas/${next.id}`}
        ...
        <aside className="hidden lg:block space-y-3">
          ...
          {trilha.modulos.map((modulo) => (
            <div key={modulo.id} className="solid-surface p-3 space-y-1">
              <p className="text-xs font-semibold text-muted-foreground mb-2">{modulo.titulo}</p>
              {modulo.aulas.map((a) => (
                <Link
                  key={a.id}
                  href={`/aulas/${a.id}`}
