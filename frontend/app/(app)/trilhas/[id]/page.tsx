...
import { ChevronLeft, CheckCircle2, Circle, Clock, ChevronDown } from "lucide-react";
...
export default function TrilhaDetalhePage() {
  ...
  const aulasConcluidas = allAulas.filter((a) => a.concluida).length;
  const pct = Math.round(trilha.progresso_pct);
  const nextAula = allAulas.find((a) => !a.concluida);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {trilha.descricao && <p className="mt-1 text-sm text-white/80 max-w-lg">{trilha.descricao}</p>}
      ...
      {trilha.modulos.length === 0 ? (
        ...
        <Accordion defaultValue={trilha.modulos[0]?.id ? [trilha.modulos[0].id] : []}>
                  ...
                  {modulo.aulas.map((aula) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="flex-1 line-clamp-1">{aula.titulo}</span>
                        {isNext && (
