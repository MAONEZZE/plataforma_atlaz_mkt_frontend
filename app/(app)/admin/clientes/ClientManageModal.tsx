...
export function ClientManageModal({ open, onOpenChange, client, onSuccess }: ClientManageModalProps) {
  ...
  useEffect(() => {
    ...
    const loaded = detailQuery.data.events.map((e) => ({ id: e.id, title: e.title, date: e.date }));
    setDates(loaded);
  ...
  const mut = useMutation({
    mutationFn: async (d: ClientFormInput) => {
      if (client) {
        ...
        const toDelete = initialDates.filter((d0) => d0.id && !dates.some((d1) => d1.id === d0.id));
        const toCreate = dates.filter((d1) => !d1.id);
        await Promise.allSettled([
          ...toDelete.map((d0) => adminEventos.remove(d0.id!)),
          ...toCreate.map((d1) =>
            adminEventos.create({ title: d1.title, date: d1.date, client_id: client.id }),
          ),
        ]);
        return;
      }
      ...
      if (dates.length > 0) {
        const results = await Promise.allSettled(
          dates.map((dt) =>
            adminEventos.create({ title: dt.title, date: dt.date, client_id: created.id }),
          ),
        );
        const failedTitles = results
          .map((r, i) => (r.status === "rejected" ? dates[i].title : null))
          .filter((t): t is string => t !== null);
        if (failedTitles.length > 0) {
          toast.warning(`Cliente criado, mas as datas "${failedTitles.join(", ")}" não foram salvas.`);
        }
      }
    },
    onSuccess: () => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl glass" showCloseButton={false}>
        <DialogHeader>
        ...
        <form
          ...
          className="space-y-5 max-h-[70vh] overflow-y-auto px-0.5 py-0.5"
          noValidate
        >
          {/* Dados */}
          ...
          <div className="space-y-3">
            <p className="eyebrow">Datas do cliente</p>
